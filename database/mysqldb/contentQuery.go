...
const (
	InsertContentQuery      = "INSERT INTO content (title, created_date, hits, meta_author_name, meta_desc, meta_key_words, meta_robot_key_words, text, client_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) "
	UpdateContentQuery      = "UPDATE content set title = ?, modified_date = ?, hits = ?, meta_author_name = ?, meta_desc = ?,meta_key_words = ?, meta_robot_key_words = ?, text = ? where id = ? and client_id = ? "
